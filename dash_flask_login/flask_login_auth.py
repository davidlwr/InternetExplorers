...
class FlaskLoginAuth():
    def __init__(self, app, use_default_views=False, users=None, auto_hash=True, hash_function=None):
        ...
        if use_default_views:
            ...
            if not users:
                warnings.warn(
                    '''No connection string or list of users supplied, defaulting to single user environment with USER_NAME: admin and PASSWORD: admin.\nYou will be unable to change this password or add other users.''')
            ...
            # Check if users is a list, if so, check if it's a list of string or list of User objects
                ...
                warnings.warn(
                    '''By simply supplying a list of authorized users, your users will be unable to change their passwords.''')
                # If all objects are strings, create a UserMap from the strings
                if all(isinstance(users[i], tuple) for i in range(len(users))):
                    self.users = UserMap(
                        [DefaultUser(users[i][0], users[i][1], self.auto_hash, self.hash_function) for i in
                         range(len(users))])
                ...
                # If all objects are a subclass of UserMixin, create a UserMap of the objects
                    ...
                    if self.auto_hash:
                        warnings.warn(
                            '''Supplying a list of UserMixin subclass objects does not allow automated password hashing.  Please ensure passwords are safely stored''')
                    ...
                    raise TypeError(
                        "All objects in the list must be a tuple of form (USER_NAME, PASSWORD) or a subclass of flask_login.UserMixin")
                ...
                self.users = UserMap(
                    [DefaultUser(result_set[i][0], result_set[i][1], self.auto_hash, self.hash_function) for i in
    ...
    "users" must be one of %s, %s, %s, or %s.  Provided type was %s''' % (
                type('s'), type([]), sqlite3.Connection, type(None), type(users))
                                )
