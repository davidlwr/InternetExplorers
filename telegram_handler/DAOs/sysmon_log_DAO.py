...
class sysmon_log_DAO(object):
    ...
    def delete_log(uuid):
        '''
        1. Remove logs to turn sensor to WARNING
        INSERTs a log entry into the database

        Returns success boolean
        '''
        print("deleting")
        query = "DELETE FROM {} WHERE uuid = %s".format(sysmon_log_DAO.table_name)
        values = (uuid)
        # Get connection
    ...
    @staticmethod
    def insert_log(uuid, node_id, event, key, timestamp):
        '''
        INSERTs a log entry into the database
        ...
        query = "INSERT INTO {} VALUES('{}', '{}', '{}', '{}', '{}')"                        \
                    .format(sysmon_log_DAO.table_name, uuid, node_id,                \
                            event, key, timestamp)
