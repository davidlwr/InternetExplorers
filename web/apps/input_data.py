...
class input_data(object):
    ...
    def get_num_visits_by_date(start_date=input_raw_min_date, end_date=input_raw_max_date,
        ...
        # print(current_data)
        if time_period == 'Day':
            current_data = current_data.loc[(current_data['recieved_timestamp'].dt.time >= input_data.daytime_start)
                                            & (current_data['recieved_timestamp'].dt.time < input_data.daytime_end)]
        elif time_period == 'Night':
            current_data = current_data.loc[(current_data['recieved_timestamp'].dt.time < input_data.daytime_start)
                                            | (current_data['recieved_timestamp'].dt.time > input_data.daytime_end)]
            if offset:
                # shift the date of those at night to be one day before
                current_data['recieved_timestamp'] = np.where(current_data['recieved_timestamp'].dt.time < input_data.daytime_start,
                                                              current_data['recieved_timestamp'] - datetime.timedelta(days=1),
                                                              current_data['recieved_timestamp'])

        # group by date only
        current_data['gw_date_only'] = current_data['recieved_timestamp'].apply(input_data.date_only)
    ...
    def get_relevant_data(input_location, start_date, end_date, resident_id=1, grouped=False):
        ...
        if grouped and ('m-02' in input_location):
            relevant_data = input_data.get_grouped_data(relevant_data)
    ...
    def get_location_options():
        locations = input_data.input_raw_data['uuid'].unique().tolist()
        output = {}
    ...
    def get_average_longest_sleep(resident_id, start_date, end_date, use_door=False):
        ...
        start_time = input_data.default_sleep_start
        end_time = input_data.default_sleep_end
        ...
        for single_date in date_range:
            if use_door:
                ...
                start_time = input_data.default_sleep_start  # replace with use door
                end_time = input_data.default_sleep_end  # replace with use door
    ...
    def get_vital_signs_indicator(resident_id, current_sys_time=None):
        ...
        if isinstance(past_week_breathing_df, str) or past_week_breathing_df.empty:
            ...
            if (breathing_mean + breathing_h) < input_data.normal_lower_bound_rr:
                alerts_of_interest.append(f"Night respiratory rate from previous week significantly lower than normal (<{input_data.normal_lower_bound_rr})")
            elif (breathing_mean - breathing_h) > input_data.normal_upper_bound_rr:
                alerts_of_interest.append(f"Night respiratory rate from previous week significantly higher than normal (>{input_data.normal_upper_bound_rr})")
        ...
        if isinstance(previous_weeks_breathing_df, str) or past_week_breathing_df.empty:
            ...
            if not (isinstance(past_week_breathing_df, str) or past_week_breathing_df.empty):
                ...
                previous_weeks_breathing_df['date_only'] = previous_weeks_breathing_df['reading_timestamp'].apply(input_data.date_only)
        ...
        if isinstance(previous_weeks_heartbeat_df, str) or previous_weeks_heartbeat_df.empty:
            ...
            if not (isinstance(past_week_heartbeat_df, str) or past_week_heartbeat_df.empty):
                ...
                previous_weeks_heartbeat_df['date_only'] = previous_weeks_heartbeat_df['reading_timestamp'].apply(input_data.date_only)
